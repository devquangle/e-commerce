...
export const useGetVoucherForUser = (code?: string) => {
  const { isInitialized, userInfo } = useAuth();
  return useQuery<VoucherUserResponse[]>({
    queryKey: ["voucher-user", userInfo?.code, code],
    queryFn: () => VoucherService.getVoucherForUser(code),
    enabled: isInitialized && !!userInfo,
  });
};

export const useGetVoucherForUserMutation = () => {
  return useMutation({
    mutationFn: (code: string) => VoucherService.getVoucherForUser(code),
    onError: (error: unknown) => {
      let errorMsg = "Đã xảy ra lỗi khi lấy thông tin voucher.";
      if (axios.isAxiosError(error)) {
        errorMsg = error.response?.data?.message || error.message || errorMsg;
      } else if (error instanceof Error) {
        errorMsg = error.message;
