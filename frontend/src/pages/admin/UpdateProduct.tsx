  ...
  return (
    <section className="space-y-6 p-4 md:p-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Cập nhật sách</h1>
          <p className="text-sm text-gray-500">Chỉnh sửa thông tin sản phẩm sách hiện có.</p>
        </div>
              ...
              {form.coverImages.filter(Boolean).length > 0 ? (
                form.coverImages
                  ...
                  .map((image, index) => (
                    ...
                    >
                      <img src={image} alt={`Ảnh sản phẩm ${index + 1}`} className="h-full rounded object-contain" />
                    </div>
                  ))
              ) : (
                <p className="col-span-2 text-center text-sm text-gray-500">Chưa có ảnh để xem trước.</p>
              )}
            </div>
          </div>
        </div>
