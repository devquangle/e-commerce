...
export default function MenuItemMobile() {
  ...
  const genres = genresData.filter((g) => (g.bookCount || 0) > 0);
  const authors = authorsData.filter((a) => (a.bookCount || 0) > 0);
    ...
    <ul className="md:hidden space-y-1">
              ...
              {loadingGenres ? (
                <div className="space-y-2">
                  {[1, 2, 3].map((i) => (
                    <div
                      key={i}
                      className="h-10 bg-slate-200/50 rounded-lg animate-pulse"
                    ></div>
                  ))}
                </div>
              ) : (
            ...
            <div>
              ...
              <div className="grid grid-cols-1 gap-2">
                <Link
                  ...
                  className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-white hover:bg-amber-50 hover:text-amber-700 transition-all shadow-sm border border-slate-100 group"
                >
                  <div className="w-7 h-7 rounded-full bg-amber-100 text-amber-600 flex items-center justify-center shrink-0">
                    <Star size={14} className="fill-current" />
                  </div>
                  <span className="font-bold text-sm text-slate-700 group-hover:text-amber-700">
                    Bán chạy nhất
                  </span>
                </Link>
                <Link
                  to="/products?sort=newest"
                  className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-white hover:bg-blue-50 hover:text-blue-700 transition-all shadow-sm border border-slate-100 group"
                >
                  <div className="w-7 h-7 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center shrink-0">
                    <Zap size={14} className="fill-current" />
                  </div>
                  <span className="font-bold text-sm text-slate-700 group-hover:text-blue-700">
                    Sách mới
                  </span>
                </Link>
                <Link
                  to="/products?discount=true"
                  className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-white hover:bg-red-50 hover:text-red-700 transition-all shadow-sm border border-slate-100 group"
                >
                  ...
                  <span className="font-bold text-sm text-slate-700 group-hover:text-red-700">
                    Khuyến mãi
                  </span>
                </Link>
              </div>
      ...
      {menuItems.map((item) => (
        <li key={item.id}>
          <Link
            to={item.path}
            className="flex items-center py-3 px-4 font-bold text-slate-800 hover:bg-slate-50 hover:text-indigo-600 rounded-xl transition-all duration-200"
          >
            {item.label}
          </Link>
        </li>
